  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        const response = await axios.get(`http://ec2-3-19-248-251.us-east-2.compute.amazonaws.com:8080/api/users/username/${formData.username}`);
        if (response.data) {
        ...
        const response = await axios.post('http://ec2-3-19-248-251.us-east-2.compute.amazonaws.com:8080/api/users', formData);
        if (response.data) {
          localStorage.setItem('user', JSON.stringify(response.data));
          navigate('/upload');
        }
